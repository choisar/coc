...
document.addEventListener('DOMContentLoaded', function() {
  // 버튼에 대한 참조를 올바르게 조정합니다.
  // "맨 위로 가기" 버튼이 첫 번째 버튼으로,
  // "맨 아래로 가기" 버튼이 두 번째 버튼으로 지정되었음을 가정합니다.
  var moveToTopButton = document.querySelectorAll('button.move')[0]; // "맨 위로 가기" 버튼으로 수정
  var moveToBottomButton = document.querySelectorAll('button.move')[1]; // "맨 아래로 가기" 버튼으로 수정

  // "맨 위로 가기" 버튼 클릭 이벤트: 올바른 기능을 수행하도록 유지
  moveToTopButton.addEventListener('click', function() {
    window.scrollTo({ top: 0, behavior: 'smooth' });
  });

  // "맨 아래로 가기" 버튼 클릭 이벤트: 올바른 기능을 수행하도록 수정
  moveToBottomButton.addEventListener('click', function() {
    window.scrollTo({ top: document.body.scrollHeight, behavior: 'smooth' });
  });

  // 초기 상태에서 "맨 아래로 가기" 버튼을 활성화 상태로 만듭니다.
